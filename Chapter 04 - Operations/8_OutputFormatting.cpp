//IO Manipulators : are helper functions that make it possible to control input/output streams using operator<< or operator>>.
...
/*
    statments
    std::endl -> print new line
    
    std::flush -> causes immediate sending of data to the device connected to the stream
    
    std::setw() -> changes the width of the next input/output field

    std::internal/left/right -> sets the placement of fill characters

    std::setfill -> changes the fill character

    std::boolalpha/noboolalpha -> switches between textual and numeric representation of booleans

    std::showpos/noshowpos -> controls whether the + sign used with non-negative numbers

    std::dec/hex/oct -> changes the base used for integer I/O, not for floating points.

    std::showbase/noshowbase -> controls whether prefix is used to indicate numeric base

    std::uppercase/nouppercase -> controls whether uppercase characters are used with some output formats

    std::fixed/scientific/hexfloat/defaultfloat -> changes formatting used for floating-point I/O

    std::setprecision -> changes floating-point precision

    std::showpoint/noshowpoint -> controls whether decimal point is always included in floating-point representation
*/
